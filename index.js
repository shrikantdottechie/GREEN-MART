...
app.get("/user/sign-up", function (req, res, next) {
    var messages = req.flash('error');
    res.render('user/sign-up', { csrfToken: req.csrfToken(), isAdminField: true, messages: messages, hasErrors: messages.length > 0 });
});
...
app.post("/user/sign-in", passport.authenticate('local.sign-in', {
    successRedirect: '/',
...
app.get('/add-to-cart/:id/:qty', async (req, res) => {
    console.log("add to cart api" + req.session.id);
...
app.get('/cart', async (req, res) => {
    ...
    res.render('cart', { csrfToken: req.csrfToken(), products: cart.lookupItems(), totalPrice: parseFloat(cart.totalPrice).toFixed(2) });
});

//route to render the checkout page
app.post('/checkout', function (req, res) {
    console.log("checkout api" + req.headers.csrfToken);
    if (!req.session.cart) {
        return res.render('cart', { csrfToken: req.csrfToken(), products: null });
    }

    var productArray = req.body.products;
    var grandTotal = req.body.grandTotal;

    res.render('checkout', { csrfToken: req.csrfToken(), grandTotal: grandTotal, products: productArray });
...
app.get("/admin/add", isLoggedIn, (req, res) => {
    //console.log("admin add get csrf: "+req.csrfToken());
    res.render("admin_add_product", { csrfToken: req.csrfToken() });

})

//route for adding a new product by admin
app.post("/products/add", isLoggedIn, upload.single("file"), async (req, res) => {
    //console.log("products add get csrf: "+req.csrfToken());
    if (req.file) {
    ...
    await add_product.save().then((result) => {
        res.render("/" + req.body.category, { csrfToken: req.body._csrf });
    }).catch((error) => {
        res.send(error)
    })
})
...
app.get("/products/:id/show", isLoggedIn, (req, res) => {
    products.findById({ _id: req.params.id }).then((product) => {
        res.render("admin_show_product", { csrfToken: req.csrfToken(), product: product })
    }).catch((error) => {
        res.status(500).send(error)
    })
})
...
app.get("/products/:id", isLoggedIn, (req, res) => {
    products.findById({ _id: req.params.id }).then((product) => {
        ...
        res.render("admin_edit_product", { csrfToken: req.csrfToken(), product })
...
app.get("/admin/update", isLoggedIn, (req, res) => {
    ...
    res.render("admin_search_filter_product", { csrfToken: req.csrfToken() });
...
app.post("/products/search", isLoggedIn, (req, res) => {
    ...
    if (category === "all") {
        ...
        product.find({ title: { $regex: title }, category: { $regex: category } }).then((products) => {
            res.send({ products: products, csrfToken: req.csrfToken() })
        }).catch((error) => {
            res.send(error)
        })
    }
...
app.post("/checkout/submitOrder", async (req, res) => {
    ...
    await new_order.save().then(() => {
        res.render("show_order", { csrfToken: req.csrfToken(), order: order_details });
        //req.session.destroy();
